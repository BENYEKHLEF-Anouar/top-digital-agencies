        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-5 max-w-5xl mx-auto">
            ...
            if ( $picks_query->have_posts() ) :
                while ( $picks_query->have_posts() ) : $picks_query->the_post();
                    ...
                    ?>
                    <div class="card-hover bg-white border border-slate-200 rounded-xl p-5 flex flex-col justify-between cursor-pointer shadow-sm" onclick="window.location.href='<?php the_permalink(); ?>'">
                        <div>
                            <div class="flex justify-between items-start mb-3">
                                <?php if ( $logo_image ) : ?>
                                    <img src="<?php echo esc_url( $logo_image ); ?>" alt="<?php the_title_attribute(); ?>" class="w-12 h-12 rounded-lg object-cover border border-slate-100 bg-white">
                                <?php else : ?>
                                    <div class="w-12 h-12 rounded-lg flex items-center justify-center bg-brand-600 text-white font-bold text-xs uppercase"><?php echo esc_html( $logo_text ? $logo_text : substr( get_the_title(), 0, 2 ) ); ?></div>
                                <?php endif; ?>
                                
                                <span class="bg-amber-700 inline-flex items-center gap-0.5 text-white text-[9px] font-bold px-2 py-0.5 rounded font-mono uppercase tracking-wide">
                            ...
                            <p class="text-[13px] text-slate-500 leading-relaxed truncate-3-lines mb-4"><?php echo esc_html( get_the_excerpt() ); ?></p>
                        </div>
